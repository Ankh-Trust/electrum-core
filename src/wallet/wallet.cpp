...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    ...
    std::map<CElectrumAddress, double> splitMap;
    double nAccumulatedFee = 0.0;

    CElectrumAddress poolFeeAddress(GetArg("-pooladdress", ""));
    ...
    else if (GetArg("-stakingaddress", "") != "")
    {
        ...
        if (stakingAddress.read(GetArg("-stakingaddress", "")))
        {
            try {
                ...
                if (find_value(addressMap, CElectrumAddress(key.GetPubKey().GetID()).ToString()).isStr())
                    lookForKey = CElectrumAddress(key.GetPubKey().GetID()).ToString();

                if (find_value(addressMap, CElectrumAddress(key.GetPubKey().GetID()).ToString()).isObject())
                {
                    find_value(addressMap, CElectrumAddress(key.GetPubKey().GetID()).ToString()).getObjMap(splitObject);
                ...
                {
                    address = CElectrumAddress(find_value(addressMap, lookForKey).get_str());
                ...
                else if(find_value(addressMap, lookForKey).isObject())
                {
                    ...
                    for ( const auto &pair : splitObject ) {
                        address = CElectrumAddress(pair.first);
        ...
        {
            address = CElectrumAddress(GetArg("-stakingaddress", ""));
            if (address.IsValid()) {
    ...
    txNew.nVersion = /*IsCommunityFundEnabled(chainActive.Tip(),Params().GetConsensus())*/ true ? CTransaction::TXDZEEL_VERSION_V2 : CTransaction::TXDZEEL_VERSION;
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CElectrumAddress(CScriptID(redeemScript)).ToString();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-electrum-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CElectrumAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CElectrumAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CElectrumAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CElectrumAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CElectrumAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CElectrumAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CElectrumAddress(dest).ToString(), key);
}
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-stakingaddress", strprintf(_("Specify a customised electrum address to accumulate the staking rewards.")));
...
std::string CWallet::formatDisplayAmount(CAmount amount) {
    ...
    string _ae_amount = n.str();
    if(_ae_amount.at(_ae_amount.length()-1) == '.') {
        _ae_amount = _ae_amount.substr(0, _ae_amount.size()-1);
    }
