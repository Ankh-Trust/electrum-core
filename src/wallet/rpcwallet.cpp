...
UniValue getcoldstakingaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2)
        throw runtime_error(
            ...
            "3. \"voting\"          (string, optional) The electrum voting address.\n\n"
            "\nExamples:\n"
    ...
    if (params.size() == 3)
    {
        CElectrumAddress votingAddress(params[2].get_str());
        CKeyID votingKeyID;
        if (!votingAddress.IsValid() || !votingAddress.GetKeyID(votingKeyID))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Voting address is not a valid Electrum address");

        votingAddress.GetKeyID(votingKeyID);

        return CElectrumAddress(stakingKeyID, spendingKeyID, votingKeyID).ToString();
    }
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw runtime_error(
            "sendtoaddress \"electrumaddress\" amount ( \"comment\" \"comment-to\" \"strdzeel\" subtractfeefromamount )\n"
...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    CElectrumAddress paddress(paymentAddress);
    if (!paddress.IsValid())
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Electrum address for payment");

    CElectrumAddress oaddress(ownerAddress);
    if (!oaddress.IsValid())
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Electrum address for the owner");
...
UniValue proposeconsensuschange(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || !params[0].isNum() || !params[1].isNum())
        throw runtime_error(
            ...
            "\nCreates a proposal to the DAO for changing a consensus paremeter. Min fee of " + FormatMoney(nMinFee) + "0AE is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
...
UniValue createconsultation(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1)
        throw runtime_error(
            ...
            "\nCreates a consultation for the DAO. Min fee of " + FormatMoney(GetConsensusParameter(Consensus::CONSENSUS_PARAM_CONSULTATION_MIN_FEE, view)) + "0AE is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            ...
            "\"  strDZeel: string }\"            (string) The attached strdzeel property.\n"
            "\nExamples:\n"
            + HelpExampleCli("createconsultation", "\"Who should be the CEO of Electrum? /s\" 1 1")
            + HelpExampleCli("createconsultation", "\"How much should Electrum's CEO earn per month? /s\" 1000 5000 true")
        );
...
UniValue createconsultationwithanswers(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2)
        throw runtime_error(
            ...
            "\"  strDZeel: string }\"            (string) The attached strdzeel property.\n"
            "\nExamples:\n"
            + HelpExampleCli("createconsultationwithanswers", "\"Who should be the CEO of Electrum? /s\" \"[\\\"Craig Wright\\\",\\\"Loomdart\\\"]\"")
        );
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    CElectrumAddress address(proposal.GetOwnerAddress());
...
UniValue proposeanswer(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2)
        throw runtime_error(
            ...
            "\nProposes an answer for an already existing consultation of the DAO. Min fee of " + FormatMoney(GetConsensusParameter(Consensus::CONSENSUS_PARAM_CONSULTATION_ANSWER_MIN_FEE, view)) + "0AE is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    if (fAllAccounts || (account == strAccount))
    {
        ...
        entry.pushKV("canStake", (::IsMine(*pwalletMain, r.destination) & ISMINE_STAKABLE ||
                                          (::IsMine(*pwalletMain, r.destination) & ISMINE_SPENDABLE &&
                                           !CElectrumAddress(r.destination).IsColdStakingAddress(Params()) &&
                                           !CElectrumAddress(r.destination).IsColdStakingv2Address(Params()))) ? true : false);
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return _("wallet encrypted; Electrum server stopping, restart to run with encrypted wallet.");
}
...
UniValue encrypttxdata(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return _("txdata encrypted; Electra server stopping, restart to run with encrypted txdata.");
}
...
UniValue listproposals(const UniValue& params, bool fHelp)
{
    ...
    if(view.GetAllProposals(mapProposals))
    {
        for (CProposalMap::iterator it = mapProposals.begin(); it != mapProposals.end(); it++)
        {
            ...
            if (showMine)
            {
                CTxDestination address(CElectrumAddress(proposal.GetOwnerAddress()).Get());
                isminefilter mine = IsMine(*pwalletMain, address);
...
{ //  category              name                        actor (function)           okSafeMode
  //  --------------------- ------------------------    -----------------------    ----------
    ...
    { "ankhfund",           "donatefund",               &donatefund,               false },
    { "ankhfund",           "createpaymentrequest",     &createpaymentrequest,     false },
    { "ankhfund",           "createproposal",           &createproposal,           false },
    ...
    { "ankhfund",           "proposalvote",             &proposalvote,             false },
    { "ankhfund",           "proposalvotelist",         &proposalvotelist,         false },
    { "ankhfund",           "listproposals",            &listproposals,            true  },
    { "ankhfund",           "paymentrequestvote",       &paymentrequestvote,       false },
    { "ankhfund",           "paymentrequestvotelist",   &paymentrequestvotelist,   false },
    { "ankhfund",           "proposalvote",             &proposalvote,             false },
    { "ankhfund",           "proposalvotelist",         &proposalvotelist,         false },
