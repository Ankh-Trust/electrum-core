...
#if defined(NDEBUG)
# error "Electrum cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Electrum Signed Message:\n";
        ...
        if (fAddressIndex)
        {
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
                    ...
                    CElectrumAddress address(destination);
                    ...
                    CElectrumAddress addressStaking, addressVoting, addressSpending;
...
void ThreadScriptCheck() {
    RenameThread("electrum-scriptch");
            ...
            if (fAddressIndex || fSpentIndex)
            {
                for (size_t j = 0; j < tx.vin.size(); j++) {
                    ...
                    } else if (prevout.scriptPubKey.IsColdStaking())
                    {
                        ...
                        CElectrumAddress addressStaking, addresssSpending;
                    ...
                    else if (prevout.scriptPubKey.IsColdStakingv2())
                    {
                        ...
                        CElectrumAddress addressStaking, addressVoting, addresssSpending;
        ...
        if (fAddressIndex) {
            for (unsigned int k = 0; k < tx.vout.size(); k++) {
                ...
                } else if (out.scriptPubKey.IsColdStaking())
                {
                    ...
                    CElectrumAddress addressSpending, addressStaking;
                    int type = 0;
                    CTxDestination destination;
                    ExtractDestination(out.scriptPubKey, destination);
                    CElectrumAddress address(destination);
                ...
                else if (out.scriptPubKey.IsColdStakingv2())
                {
                    ...
                    CElectrumAddress addressSpending, addressStaking, addressVoting;
    ...
    for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
        ...
        if(nAccValue > nPOWBlockReward && block.vtx[0].vout[i].nValue > 0) {
            ...
            if(metadata[nPaymentRequestsCount].isStr())
            {
                ...
                if(block.vtx[0].vout[i].nValue != prequest.nAmount || (fv452Fork && prequest.GetLastState() != DAOFlags::ACCEPTED) || proposal.GetPaymentAddress() != CElectrumAddress(address).ToString())
...
//
// Commented - Electrum uses now version control
//
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of Electrum, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of Electrum, please update.";
            fObsolete = true;
        }
