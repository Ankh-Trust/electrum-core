...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total electrum balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"electrumaddress\"\n"
            "\nReturn information about the given electrum address.\n"
            "\nArguments:\n"
            "1. \"electrumaddress\"     (string, required) The electrum address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"electrumaddress\", (string) The electrum address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"electrumaddress\", (string) The electrum staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"electrumaddress\", (string) The electrum spending address part of a cold staking address\n"
            "  \"votingaddress\" : \"electrumaddress\", (string) The electrum voting address part of a cold staking v2 address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            CElectrumAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CElectrumAddress spendingAddress;
            ...
            CElectrumAddress spendingAddress;
            address.GetSpendingAddress(spendingAddress);
            CElectrumAddress votingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: Electrum address and we have full public key:
        CElectrumAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are electrum addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) electrum address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CElectrumAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CElectrumAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"electrumaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"electrumaddress\"  (string, required) The electrum address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CElectrumAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CElectrumAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CElectrumAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
bool getAddressesFromParamsForHistory(const UniValue& params, std::vector<std::pair<std::pair<uint160, uint160>, AddressHistoryFilter>> &addresses)
{
    if (params[0].isStr()) {
        ...
        {
            CElectrumAddress addressVoting, addressStaking, addressSpending;
        ...
        {
            CElectrumAddress addressStaking, addressSpending;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            if (address.IsColdStakingv2Address(Params()))
            {
                CElectrumAddress addressSpending, addressVoting, addressStaking;
                uint160 hashBytes, hashBytesSpending, hashBytesVoting, hashBytesStaking;
                int type = 0;
...
UniValue getaddresshistory(const UniValue& params, bool fHelp)
{
    ...
    std::map<CElectrumAddress, balStruct> balance;
