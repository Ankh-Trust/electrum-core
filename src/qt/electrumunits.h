...
/** Electrum unit definitions. Encapsulates parsing and formatting
   and serves as list model for drop-down selection boxes.
*/
class ElectrumUnits: public QAbstractListModel
{
    Q_OBJECT

public:
    explicit ElectrumUnits(QObject *parent);

    /** Electrum units.
      @note Source: https://en.electrum.it/wiki/Units . Please add only sensible ones
     */
    enum Unit
    {
        // Base currencies
    ...
    //! Format as string
    static QString format(int unit, const CAmount& amount, bool plussign=false, SeparatorStyle separators=separatorStandard, bool fPretty=false);
...
typedef ElectrumUnits::Unit ElectrumUnit;

#endif // ELECTRUM_QT_ELECTRUMUNITS_H
