    ...
    comboAddress(new QComboBox)
{
    ...
    {
        comboAddress->insertItem(i++, it.first +" ("+ QString::fromStdString(FormatMoney(it.second)) +" 0AE)",it.first);
    }
...
void SplitRewardsDialog::showFor(QString sin)
{
    ...
    for (auto &key: j.keys())
    {
        CElectrumAddress address(key.toStdString());
        double amount;
        ...
        if (address.IsValid())
        {
            ...
            if (teamAddresses.count(key))
            {
                if (teamAddresses[key] == "Ankh Fund")
                    nCFundContribution += PercentageToElectrum(amount);
                else
                    descs << QString::fromStdString(FormatMoney(PercentageToElectrum(amount))) + " to " + teamAddresses[key];
                jDup[teamAddresses[key]] = amount;
            }
            else {
                descs << QString::fromStdString(FormatMoney(PercentageToElectrum(amount))) + " to " + key;
                jDup[key] = amount;
            }
    ...
    strDesc->setText(tr("For each block, %1 0AE will go to the Ankh Fund, %2 and %3 will be accumulated in your own address").arg(QString::fromStdString(FormatMoney(nCFundContribution)), descs.join(", "), QString::fromStdString(FormatMoney(PercentageToElectrum(availableAmount)))));
...
void SplitRewardsDialog::onAdd()
{
    ...
    if (!CElectrumAddress(address.toStdString()).IsValid())
    {
        QMessageBox msgBox(this);
        msgBox.setText(tr("Invalid Electrum Address"));
        msgBox.addButton(tr("Ok"), QMessageBox::AcceptRole);
        msgBox.setIcon(QMessageBox::Warning);
        msgBox.setWindowTitle("Error");
        msgBox.exec();
