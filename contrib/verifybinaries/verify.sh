...
###   This script attempts to download the signature file SHA256SUMS.asc from electrum.org
...
WORKINGDIR="/tmp/electrum"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://electrum.org/bin/"
VERSIONPREFIX="electrum-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'electrum-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Electrum Core binary release signing key installed?"
   fi
