...
function _ae_cli {
	$navpath/electrum-cli -datadir=${array_data[$1]} -rpcport=${array_rpc_port[$1]} -devnet $2 2> /dev/null
}
...
function terminate {
	...
	then
		...
		do
			...
			_ae_cli $i listproposals > ${array_data[$i]}/devnet/listproposals.out
			_ae_cli $i listconsultations > ${array_data[$i]}/devnet/listconsultations.out
...
function connect_nodes {
	_ae_cli "$1" "addnode 127.0.0.1:$( bc <<< 10000+$2 ) add"
	_ae_cli "$1" "addnode 127.0.0.1:$( bc <<< 10000+$2 ) onetry"
}

function disconnect_nodes {
	_ae_cli "$1" "addnode 127.0.0.1:$( bc <<< 10000+$2 ) remove"
	_ae_cli "$1" "disconnectnode 127.0.0.1:$( bc <<< 10000+$2 )"
}
...
function check_connection {
	...
	do
		...
		check=$(_ae_cli ${connection_pair[0]} getpeerinfo | grep "\"addr\": \"127.0.0.1:$( bc <<< "10000+${connection_pair[1]}" )\"")
		if [ -z "$check" ];
		then
...
function start_random_stopped_nodes {
	...
	then
		...
		echo Waiting 30 seconds for electrumd...
		sleep 30
		for j in ${!local_array[@]};
		do
			connect_node_to_network $j
			echo "Nodes $j balance: $(_ae_cli $j getbalance) t0AE"
		done
	fi
	echo Currently active nodes are: ${array_active_nodes[@]} inactive ones are: ${array_stopped_nodes[@]}
}
...
function wait_until_sync {
	...
	for i in ${local_array[@]};
	do
		local_array_best_hash[$i]=$(_ae_cli $i getbestblockhash)
#		echo best block hash of node $i:
#		echo "$(_ae_cli $i getbestblockhash)"
	done
...
#		echo array topology node pairs stopped is "${array_topology_node_pairs_stopped[@]}"
		...
		$(_ae_cli $node "generate 1")
		echo now on blcok $(_ae_cli $node "getblockcount")
...
function assert_state {
	...
	for i in ${local_array[@]};
	do
		local_array_statehash[$i]=$(_ae_cli $i getcfunddbstatehash)
	done
...
function random_transactions {
	...
	if [ $dice -lt 75 ];
		...
		out=$(_ae_cli ${array_stressing_nodes[$node_send]} "sendtoaddress ${array_address[$node_receive]} $transaction_amount")
	fi
}
...
function dice_proposal {
	...
	then
		...
		address=$(_ae_cli ${array_stressing_nodes[$node]} getnewaddress)
		out=$(_ae_cli ${array_stressing_nodes[$node]} "createproposal $address $amount $deadline \"$random_sentence\"")
	fi
	...
	then
		proposals=$(_ae_cli ${array_stressing_nodes[$node]} "listproposals mine"|jq -r ".[]|.hash"|tr "\n" " ")
		array_proposals=($proposals)
		for p in ${array_proposals[@]}
		do
			proposal=$(_ae_cli $node "getproposal $p")
			...
			then
				...
				then
					...
					out=$(_ae_cli ${array_stressing_nodes[$node]} "createpaymentrequest $hash $requestAmount \"$random_sentence\"")
				fi
			fi
		done
	fi
...
function dice_consultation {
	...
	then
		...
		out=$(_ae_cli ${array_stressing_nodes[$node]} "createconsultation $random_sentence $random_max_answer")
	fi
	...
	then
		...
		out=$(_ae_cli ${array_stressing_nodes[$node]} "createconsultationwithanswers $random_sentence [\\\"$consultation_answer\\\"] $random_max_answer $bool_user_propose_new_answer")
	fi
	...
	then
		...
		out=$(_ae_cli ${array_stressing_nodes[$node]} "createconsultation $random_sentence $random_lower_limit $random_upper_limit true")
	fi
	...
	then
		...
		out=$(_ae_cli ${array_stressing_nodes[$node]} "proposeconsensuschange $consensus $value")
#		echo "Proposing changing consensus ${consensus_parameter_name[$consensus]} from ${consensusparameter_new[$consensus]} to $value"
	...
	then
		consultations=($(_ae_cli ${array_stressing_nodes[$node]} "listconsultations"|jq -r ".[]|.hash"|tr "\n" " "))
		for c in ${consultations[@]}
		do
			consultation=$(_ae_cli $node "getconsultation $c")
				...
				then
					...
					do
						...
						then
							...
							out=$(_ae_cli ${array_stressing_nodes[$node]} "proposeanswer $hash $consensus $value")
#							echo Adding $value as a new answer to $question
				...
				then
					random_sentence=$(env LC_CTYPE=C tr -dc "a-zA-Z0-9-_\$\?" < /dev/urandom | head -c 10)
					out=$(_ae_cli ${array_stressing_nodes[$node]} "proposeanswer $hash $random_sentence")
				fi
			fi
		done
	fi
...
function voter_dice_proposal {
	...
	proposals=($(_ae_cli ${array_stressing_nodes[$node]} proposalvotelist|jq -r ".null[]|.hash"))
	for i in ${proposals[@]};
	do
		dice=$(bc <<< "$RANDOM % 2")
		if [ $dice -eq 1 ];
		then
			out=$(_ae_cli ${array_stressing_nodes[$node]} "proposalvote $i yes")
		else
			out=$(_ae_cli ${array_stressing_nodes[$node]} "proposalvote $i no")
		fi
	done

	prequests=($(_ae_cli ${array_stressing_nodes[$node]} paymentrequestvotelist|jq -r ".null[]|.hash?"))
	...
	do
		...
		if [ $dice -eq 1 ];
		then
			out=$(_ae_cli ${array_stressing_nodes[$node]} "paymentrequestvote $i yes")
		else
			out=$(_ae_cli ${array_stressing_nodes[$node]} "paymentrequestvote $i no")
		fi
	done
}
...
function voter_dice_consultation {
	...
	node=${shuffled_array[0]}
	consultations=($(_ae_cli ${array_stressing_nodes[$node]} "listconsultations"|jq -r ".[]|.hash"|tr "\n" " "))
	all_consultation_answers=($(_ae_cli ${array_stressing_nodes[$node]} "listconsultations"|jq -r ".[].answers[]|.hash"|tr "\n" " "))
	for i in ${consultations[@]};
	do
		consultation=$(_ae_cli $node "getconsultation $i")
		version=$(echo $consultation | jq -r .version)
		status=$(echo $consultation | jq -r .status)
		if [[ "$status" == "waiting for support" ]] || [[ "$status" == "waiting for support, waiting for having enough supported answers" ]];
		then
			...
			then
				...
				if [ $dice -eq 1 ];
				then
					out=$(_ae_cli ${array_stressing_nodes[$node]} "support $i")
				else
					out=$(_ae_cli ${array_stressing_nodes[$node]} "support $i false")
				fi
			else
				...
				do
					...
					if [ $dice -eq 0 ];
					then
						out=$(_ae_cli ${array_stressing_nodes[$node]} "support $k")
					else
						out=$(_ae_cli ${array_stressing_nodes[$node]} "support $k false")
					fi
				done
			fi
		...
		then
			...
			then
				...
				if [ $dice -eq 1 ];
				then
					out=$(_ae_cli ${array_stressing_nodes[$node]} "consultationvote $i $RANDOM")
				else
					out=$(_ae_cli ${array_stressing_nodes[$node]} "consultationvote $i abs")
				fi
			else
				out=$(_ae_cli ${array_stressing_nodes[$node]} "consultationvote $i remove")
				consultation_answers=($(_ae_cli $node "getconsultation $i"| jq -r ".answers[].hash" | tr "\n" " "))
				for k in ${consultation_answers[@]};
				do
					out=$(_ae_cli ${array_stressing_nodes[$node]} "consultationvote $k remove")
				done
				...
				then
					...
					then
						...
						out=$(_ae_cli ${array_stressing_nodes[$node]} "consultationvote ${consultation_answers[$yes_answer]} yes")
					fi
				else
					dice=$(bc <<< "$RANDOM % 5")
					if [ $dice -eq 1 ];
					then
						...
						do
							...
							if [ $dice -eq 1 ];
							then
								out=$(_ae_cli ${array_stressing_nodes[$node]} "consultationvote $k yes")
							fi
						done
					fi
				fi
...
function check_consensus_parameters {
	consensusparameter_tmp=($(_ae_cli $1 getconsensusparameters | tr -d "[],\n"))
...
function stress {
	...
	out=$(_ae_cli 0 "donatefund $donation")
	for i in ${array_stressing_nodes[@]};
	do
		out=$(_ae_cli $i "staking false")
		echo "Node $i balance: $(_ae_cli $i getbalance) t0AE"
	done
}

function check_node {
	blocks=`_ae_cli $2 getinfo|jq .blocks`
	block_increment=${#array_verifychain_nodes[@]}
	for i in $(seq $1 $block_increment $blocks);
	do
		verifyoutput=`_ae_cli $2 verifychain 4 $i`
	...
	verifyoutput=`_ae_cli $2 verifychain 4 0`
...
function check_cycle {
	shuffle_array "${array_stressing_nodes[@]}"
	node=${shuffled_array[0]}
	blocks=$(_ae_cli $node getinfo|jq .blocks)
...
function start_node {
        $(echo $navpath)/electrumd -datadir=${array_data[$1]} -port=${array_p2p_port[$1]} -rpcport=${array_rpc_port[$1]} -devnet -daemon -debug=dao -debug=statehash -ntpminmeasures=0 -dandelion=0 -disablesafemode -staking=0 2> /dev/null
}
...
out=$(_ae_cli 0 "generate 10")

#Distribute funds to stressing nodes
array_address=()
for n in ${!array_stressing_nodes[@]};
do
        array_address[$n]=$(_ae_cli ${array_stressing_nodes[$n]} getnewaddress)
done
for i in {1..50};
do
	for j in ${array_address[@]};
	do
		out=$(_ae_cli 0 "sendtoaddress $j 10000")
	done
	out=$(_ae_cli 0 "generate 1")
done
sleep 1
for n in ${array_stressing_nodes[@]};
do
	echo "Node $n balance: $(_ae_cli $n getbalance) t0AE"
done

#Create blocks to make block count greater than 300
blocks=$(_ae_cli 0 getblockcount)
while [ $blocks -lt 300 ];
do
	out=$(_ae_cli 0 "generate 10")
	blocks=$(_ae_cli 0 getblockcount)
...
consensus_parameter_count=$(_ae_cli 0 getconsensusparameters | jq length)
for i in $(seq 0 1 $( bc <<< "$consensus_parameter_count - 1" ));
do
	eval "consensus_parameter_name[\$i]=\$(_ae_cli 0 \"getconsensusparameters true\" | jq -r '.[] | select(.id==$i) | .desc')"
done
consensusparameter_old=($(_ae_cli 0 getconsensusparameters | tr -d "[],\n"))
consensusparameter_original=("${consensusparameter_old[@]}")
voting_cycle_length=${consensusparameter_old[0]}
blocks=$(_ae_cli 0 getinfo|jq .blocks)
...
current_block=$(_ae_cli 0 getinfo|jq .blocks)
...
while [ $wait_until_cycle -gt $this_cycle ]; do
	...
	then
		...
		then
			...
			echo Waiting 30 sec for electrumd...
			sleep 30
			connect_network "${array_topology_node_pairs[@]}"
			sleep 30
			wait_until_sync "${array_active_nodes[@]}"
	...
	echo Proposals: $(_ae_cli $node listproposals|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
	echo Payment Requests: $(_ae_cli $node listproposals|jq -c "[.[].paymentRequests|map({status:.status})]|flatten|group_by(.status)|map({status:.[0].status,count:length})|.[]")
	echo Consultations: $(_ae_cli $node listconsultations|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
	echo Consensus Parameter Change Proposed: $(_ae_cli $node listconsultations  | jq ".[].version" | grep 13 | wc -l)
...
echo Waiting until all nodes are synced
wait_until_sync "${array_active_nodes[@]}"
echo Ok! Block: $(_ae_cli $node getinfo|jq .blocks) Cycle: $(bc <<< $(_ae_cli $node getinfo|jq .blocks)/$voting_cycle_length).
echo Proposals: $(_ae_cli $node listproposals|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
echo Payment Requests: $(_ae_cli $node listproposals|jq -c "[.[].paymentRequests|map({status:.status})]|flatten|group_by(.status)|map({status:.[0].status,count:length})|.[]")
echo Consultations: $(_ae_cli $node listconsultations|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
