...
The RPM spec file provided here is for Electrum-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://electrum.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Electrum git repository](https://github.com/electrum/electrum) but are not part of
...
Currently only `Source10` is used, for the example `electrum.conf` file.
...
in the electrum git repository. At present only an SVG version of the Electrum
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the electrum
...
needed when 0.12.1 is released, a proper fix is already in the Electrum git
master branch.
...
idea to build Electrum Core against a static version of that library compiled
...
The `electrum.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `electrum-server` package. At this point, most RPM based
...
The `electrum-server` package should have SELinux support. How to properly do
...
Electrum-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/electrum/). He has been
packaging Electrum for Fedora at least since 2012.
...
4. I renamed the `electrum` package that contains the Qt GUI to `electrum-core` as
that appears to be how the general population refers to it, in contrast to
`electrum-xt` or `electrum-classic`. I wanted to make sure the general population
